...
| C-019 | Sec 6.1 (L244); Fig 4 caption (L406) | Rep-3 latency | 3 cycles / 10 ns | HLS-ESTIMATE | **no build exists** | none | **UNSUPPORTED** | No rep3 `.xo`/`.xclbin`/compile_summary in the archive despite `rep3_qec_kernel.cpp` and `rep3_link.cfg` being present as source. Not yet resynthesised this pass; next candidate for the same treatment as C-010-C-013. |
...
| ID | Location | Claim | Value | Class claimed | Class actual | Evidence | Verdict | Note |
|---|---|---|---|---|---|---|---|---|
| C-030 | Table 2 (tab:resources, L369-382) | Rep-3: 0 BRAM, 0 DSP, ≈40 FF, ≈60 LUT6, <1.0 ns | as listed | HLS-ESTIMATE | **no build exists** | none | **UNSUPPORTED** | Tilde/approximation symbols in a results table are themselves a defect (values were apparently never actually synthesised, just estimated by hand). Not yet resynthesised this pass. |
...
| C-033 | Table 2 totals row | Total: 3 BRAM18K, 0 DSP, ≈481 FF, ≈1032 LUT6 | as listed | derived | partially recomputable | arithmetic on C-030/031/032 | **NEEDS-RECOMPUTE** | With real Shor numbers (C-031) and reconstructed Steane numbers (C-032) now in hand, only Rep-3 (C-030) is still missing to recompute this row honestly. |
