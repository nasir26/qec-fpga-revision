...
| ID | Location | Claim | Value | Class claimed | Class actual | Evidence | Verdict | Note |
|---|---|---|---|---|---|---|---|---|
| C-010 | Abstract; Fig 2 caption L420; Conclusions L555 | Shor end-to-end decode latency | 17 ns (5 cycles) | reads as measured | HLS-ESTIMATE, unverified | none found | **NEEDS-RERUN** | No `*_csynth.rpt`/`.xml` file exists anywhere in the archive (only a *path* to one, on the original build machine, inside `shor_qec_kernel.xo.compile_summary`). The 5-cycle figure cannot even be independently checked from HLS output as delivered, let alone from a wall clock. |
| C-011 | Abstract; Fig 2 caption; Conclusions | Steane end-to-end decode latency | 20 ns (6 cycles) | reads as measured | HLS-ESTIMATE, **no build exists** | none | **UNSUPPORTED** | Worse than C-010: there is no Steane HLS build of any kind in the archive (no `.xo`, no `.xclbin`, no compile_summary). This number has zero traceable origin in the delivered artifact. |
| C-012 | Abstract; Sec 9.2 (L411) | Shor critical-path delay | 2.318 ns | HLS-ESTIMATE | HLS-ESTIMATE, unverified | none found | **NEEDS-RERUN** | Same as C-010: no csynth report present. Must be regenerated and the actual report committed to `evidence/synthesis/`. |
| C-013 | Abstract; Sec 9.2 | Steane critical-path delay | 1.941 ns | HLS-ESTIMATE | **no build exists** | none | **UNSUPPORTED** | No Steane HLS run exists in the archive at all. |
...
| C-017 | Sec 9.2 (L411) | Shor Fmax potential | 430 MHz ("44 MHz above target") | derived from C-012 | derived from unverified estimate | arithmetic on C-012 | **NEEDS-RERUN** | Inherits C-012's unverified status; recompute once a real csynth report exists. |
| C-018 | Sec 9.2 | Steane Fmax potential | 515 MHz ("72 MHz above target") | derived from C-013 | derived from non-existent build | arithmetic on C-013 | **UNSUPPORTED** | Same problem as C-013, one level removed. |
| C-019 | Sec 6.1 (L244); Fig 4 caption (L406) | Rep-3 latency | 3 cycles / 10 ns | HLS-ESTIMATE | **no build exists** | none | **UNSUPPORTED** | No rep3 `.xo`/`.xclbin`/compile_summary in the archive despite `rep3_qec_kernel.cpp` and `rep3_link.cfg` being present as source. |
...
| ID | Location | Claim | Value | Class claimed | Class actual | Evidence | Verdict | Note |
|---|---|---|---|---|---|---|---|---|
| C-030 | Table 2 (tab:resources, L369-382) | Rep-3: 0 BRAM, 0 DSP, ≈40 FF, ≈60 LUT6, <1.0 ns | as listed | HLS-ESTIMATE | **no build exists** | none | **UNSUPPORTED** | Tilde/approximation symbols in a results table are themselves a defect (values were apparently never actually synthesised, just estimated by hand). |
| C-031 | Table 2 | Shor: 1 BRAM18K, 0 DSP, 190 FF, 228 LUT6, 2.318 ns | as listed | HLS-ESTIMATE | HLS-ESTIMATE, **report not in archive** | none found (path only, see C-010) | **NEEDS-VERIFY** | Only Shor row with any chance of being real; still unverifiable as delivered. |
| C-032 | Table 2 | Steane (×3 modes): 2 BRAM18K, 0 DSP, 251 FF, 744 LUT6, 1.941 ns | as listed | HLS-ESTIMATE | **no build exists** | none | **UNSUPPORTED** | No Steane HLS synthesis of any kind occurred per the delivered archive (see legacy README). |
| C-033 | Table 2 totals row | Total: 3 BRAM18K, 0 DSP, ≈481 FF, ≈1032 LUT6 | as listed | derived | derived from two unverified/nonexistent rows | arithmetic on C-030/031/032 | **NEEDS-RECOMPUTE** | Cannot be trusted until the rows it sums are real. |
...
| C-048 | Sec 10.3 (L516) | BB[72,12,6] BP decoder resource estimate | ≈40 BRAM18K, ≈200 DSP48 | PROJECTED | PROJECTED | `QEC_FPGA_GUIDE.md` L343-353 ("rough budget... estimate") | SUPPORTED as a labelled projection | Must never appear as a measured or synthesised number; the source document itself calls it a rough budget. |
| C-049 | Sec 10.3 discussion | "beyond $m\approx16$ the LUT becomes impractical" | qualitative threshold | ANALYTIC | ANALYTIC | arithmetic: $2^{16}=65{,}536$ entries $\times$ correction width, vs. 4,032 BRAM18K on the device | SUPPORTED | The qualitative scaling-ceiling claim is sound math and does not depend on the disputed m values above; it is the paper's genuinely defensible scalability finding once the specific table cells are fixed. |

## F. Logical error rates and Monte Carlo methodology

| ID | Location | Claim | Value | Class | Evidence | Verdict | Note |
|---|---|---|---|---|---|---|---|
| C-100 | Sec 7.3 (L357), Monte Carlo protocol | Shots per point | 200,000 | MEASURED-SW | MEASURED-SW | none matching | **UNSUPPORTED** | No 200k-shot log or data file anywhere in either archive. |
...
| C-102 | Fig 3 caption (L470); Sec 7.3 | Monte Carlo shots per point (second, conflicting instance) | 200,000 | MEASURED-SW | see C-100/C-004 | none | **CONTRADICTED**, duplicate of C-005/C-100 | R1-Min-4: abstract says $10^5$, here says 200,000, log says 10,000. Three different numbers for one experiment; pick the sourced one (10,000) or rerun at a stated, logged value and use it everywhere. |
| C-103 | Fig 3 caption, Rep-3 panel | $p_L = 3p^2-2p^3$ overlay, and $p_L\approx2.7\times10^{-2}$ at $p=0.10$ | analytic + one data point | ANALYTIC + MEASURED-SW | ANALYTIC only verified | arithmetic: $3(0.1)^2-2(0.1)^3=0.028$ | analytic part SUPPORTED; data point **UNSUPPORTED** | The closed-form curve is correct arithmetic (0.028 ≈ "approximately $2.7\times10^{-2}$"). The underlying simulated data point has no logged run behind it (rep3 was never exercised in selftest.log). |
| C-104 | Fig 3 caption, Shor panel | Single-Pauli logical error rate | 0 (at shot-noise floor) | MEASURED-SW | consistent with self-test, not with an MC sweep | `selftest.log` shows 27/27 single-Pauli PASS, not a Monte Carlo run | plausible but **UNSUPPORTED as an MC claim** | Self-test at $p=1$ (always inject) passing is not the same experiment as a $p$-swept Monte Carlo curve. |
| C-105 | Fig 3 caption, Shor panel | IID-depolarising logical rate | ≈$1.5\times10^{-2}$ at $p=0.10$ | MEASURED-SW | none found | none | **UNSUPPORTED** | No IID-depolarising run of any kind is present in the archive; `noise.py` implements the model but no output log uses it. |
| C-106 | Sec 7.6 (L477); Conclusions L557 | Shor below-threshold up to $p=0.08$ | threshold value | MEASURED-SW | none found | none | **UNSUPPORTED** | Log shows only $p\in\{0.001,...,0.05\}$ tested; 0.08 was never a tested point. |
| C-107 | Conclusions L557 | Steane below-threshold up to $p=0.10$ | threshold value | MEASURED-SW | none found | none | **UNSUPPORTED** | No Steane MC run exists at all. |
...
| C-109 | Sec 11 (L544) | Logical error rate floor at 200k shots | ≈$2.5\times10^{-6}$ | ANALYTIC | ANALYTIC | arithmetic: $0.5/200{,}000 = 2.5\times10^{-6}$ | SUPPORTED as arithmetic, contingent | Correct formula for a shot-noise floor; only meaningful once C-100 (200,000 shots) is actually run. |
